...
RSpec.describe Item do
  ...
  describe 'Class Methods' do
    before :each do
      ...
      @hippo = @brian.items.create!(name: 'Hippo', description: "I'm a Loch Monster!", price: 50, image: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTaLM_vbg2Rh-mZ-B4t-RSU9AmSfEEq_SN9xPP_qrA2I6Ftq_D9Qw', active: false, inventory: 300)
      ...
      @discount_1 = @brian.discounts.create!(title: 'Black Friday Sale', percentage: 5, items: 20, description: 'A discount for the wise ones')
      @discount_1 = @brian.discounts.create!(title: 'Black Friday Sale', percentage: 10, items: 30, description: 'A discount for the wise ones')
    end
    ...
    it '.has_discount?' do
      expect(@hippo.has_discount?(23)).to eq(true)
      expect(@gator.has_discount?(2)).to eq(false)
    end

    it '.discount_multiplier' do
      expect(@hippo.discount_multiplier(3)).to eq(1.0)
      expect(@hippo.discount_multiplier(25)).to eq(0.95)
      expect(@hippo.discount_multiplier(37)).to eq(0.9)
    end
  end
end
